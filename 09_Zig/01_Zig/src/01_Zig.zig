...
fn calculateSumOfExtrapolatedArray(initial_arr: []i32) !i32 {
    ...
    var sum: i32 = 0;
    while (sequences.popOrNull()) |sequence1| {
        if (sequences.popOrNull()) |sequence2| {
            sum += sequence1[sequence1.len - 1] + sequence2[sequence2.len - 1];
        } else {
            sum += sequence1[sequence1.len - 1];
        }
    }
    return sum;
}
