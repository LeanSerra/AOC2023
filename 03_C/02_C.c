...
bool valid_str_number(str_number num, char **lines, size_t line_count, size_t line_length, int *row, int *column);
void append_geared(geared_numbers *g_numbers, geared_number *g_num);
...
int main(int argc, char const *argv[])
{
    ...
    geared_numbers g_nums = {NULL, 0, 0};
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        for (int j = 0; j < line_length; j++)
        {
            ...
            if (j + 1 < line_length && number.count > 0)
            {
                ...
                {
                    if (valid_str_number(number, lines, count, line_length, &col, &row))
                    {
                        geared_number *g_num = malloc(sizeof(geared_number));
                        g_num->number = str_number_to_int(number);
                        g_num->row_gear = row;
                        g_num->column_gear = col;
                        g_num->used = false;
                        append_geared(&g_nums, g_num);
                    }
    ...
    for(int i = 0; i < g_nums.count; i++)
    {
        geared_number* num = g_nums.geared_numbers[i];
        if(!num->used)
        {
            for(int j = 0; j < g_nums.count; j++)
            {
                geared_number* num2 = g_nums.geared_numbers[j];
                if (j != i && !num2->used)
                {
                    if(num->row_gear == num2->row_gear&& num->column_gear == num2->column_gear)
                    {
                        final_count += num->number*num2->number;
                        num->used = true;
                        num2->used = true;
                    }
                }
...
void append_geared(geared_numbers *g_numbers, geared_number *g_num)
{
    if (g_numbers->count >= g_numbers->capacity)
    {
        g_numbers->capacity = g_numbers->capacity == 0 ? INIT_CAP : g_numbers->capacity * 2;
        g_numbers->geared_numbers = realloc(g_numbers->geared_numbers, g_numbers->capacity * sizeof(*g_numbers->geared_numbers));
        assert(g_numbers->geared_numbers != NULL && "Not enough memory");
    }
    g_numbers->geared_numbers[g_numbers->count++] = g_num;
}